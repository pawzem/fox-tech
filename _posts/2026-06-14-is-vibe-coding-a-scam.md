...
But simple bounded contexts derived from Event storming (marked with labels on the Miro board) plus added supportive domains were generally ok. So even though I had to instruct it what I wanted for the core design it was generally useful.
...
- Use good practices from Architects I respect: Eric Evans (DDD building blocks), Adam Bien (AWS SAM setup), Tomasz Ducin (FrontEnd architecture) and after the LLM proposed the Outbox Pattern I asked it to review the plan with Oskar Dudycz's guidance
...
**Rules are enforced.** Rules are protected by either Spring Modulith, ArchUnit, a custom test and a Claude rule if broken multiple times.
...
**3 Tiers of tests.** Unit, running in one class only. Story, running on Service level with Fakes and In Memory DBs. Integration, test-containers on the whole flow

I used a similar setup successfully in the past, and it's proven to me to be awesome for LLMs as well
...
I asked the LLM to use the Party Archetype and I know how it looks. If I didn't, I would end up with a Cargo Cult Archetype.
Naming was right but for new roles LLMs created new tables instead of Party roles. So it wouldn't be flexible at all in the end. Classic, college-level setup.
...
I asked Claude to include Tomasz Ducin's work as an inspiration. I also got the north star design generated from Stitch so the setup looks reasonable.
...
I probably had too much scope for sessions when doing frontend tasks as after they were done I spent a lot of time asking Claude to fix the UI style or add missing features implemented in the backend but completely forgotten in the FE.
...
Generally at some point I grew too comfortable with Claude proposing changes to the templates and I ended up with unplanned DR on DEV. I got locked out of the DB completely during some debug session, the LLM said that the DB needed to be dropped and by the time I remembered that I have backups on DEV as well I had to wait an hour for CloudFormation to drop the DB, clean the whole stack and wait some more for redeployment. Generally fun, do not recommend. 
But it told me something. Even with a reasonable SDLC it's too easy to trust the LLM. And if I don't understand the code I would not be able to debug it on prod effectively. So I would probably ask the LLM to find the root cause. Which could damage the customer data. So this is also the lesson for my professional side. Secure by design should be the new standard and touching prod at all by hand should be a red flag. Generally what is not reproducible locally is a major bottleneck in the LLM world so instincts should be first to reproduce locally and only then change anything on any other env. It has been good practice for a while now, nevertheless security is becoming even more important now. 

## Major Changes
Overall not everything was perfect. Besides the points I already brought up I had to switch from GraalVM to SnapStart. If I had more time maybe I would have spent more time on it but I wasted so much time on the debugging of missing classes that given the whole architecture the ROI wasn't that great. Probably I should go with SnapStart from the beginning or at least choose Quarkus instead of Spring if I wanted AOT that much.

I also removed custom invoicing in favour of Stripe. Too much code to maintain and I already had Stripe for payments, so I should have checked Stripe features more before trusting the LLM with a proposal of integration.
But thanks to that I managed to prove that my backend architecture is solid. Refactoring took only a session on the backend and it was easy to spot what was changing due to nice event separation. So it felt surgical. It took a bit more on FE as I also decided to change some administrative views in favour of Stripe as well. But still easier than it would be if I had trusted Claude with the architecture.
...
LLMs are great 90% of the time and frustrating the other 10% (made up numbers). Human design and oversight is still a must for software which must be maintainable and should bring business opportunities not limitations.
I cannot ignore that I managed to do with LLMs what I couldn't do alone but at the same time it reinforces in me a view that, if affordable, LLMs will be enablers not job thieves. At least in IT.
Writing the first version of the code was never the toughest part. Figuring out what is needed. Finding tricky bugs on prod and designing new features with migrations that wouldn't destroy the current DB were always the hard part, and as it is AI is "only" a multiplier, not an autopilot.
